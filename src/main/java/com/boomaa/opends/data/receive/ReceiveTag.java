...
public enum ReceiveTag {
    ...
    CPU_INFO(0x05, Protocol.UDP, Remote.ROBO_RIO, true, (ReceiveTagBase<Float>) (packet, size) -> new TagValueMap<Float>()
    ...
    RAM_INFO(0x06, Protocol.UDP, Remote.ROBO_RIO, true, (ReceiveTagBase<Integer>) (packet, size) -> new TagValueMap<Integer>()
    ...
    PDP_LOG(0x08, Protocol.UDP, Remote.ROBO_RIO, true, (ReceiveTagBase<Integer>) (packet, size) -> {
    ...
    UDP_R2D_UNKNOWN(0x09, Protocol.UDP, Remote.ROBO_RIO, true, (ReceiveTagBase<Byte>) TagValueMap::passPackets),
    CAN_METRICS(0x0E, Protocol.UDP, Remote.ROBO_RIO, true, (ReceiveTagBase<Float>) (packet, size) -> new TagValueMap<Float>()
    ...
    RADIO_EVENTS(0x00, Protocol.TCP, Remote.ROBO_RIO, true, (ReceiveTagBase<String>) (packet, size) ->
            TagValueMap.singleton("Message", new String(packet))
    ),
    USAGE_REPORT(0x01, Protocol.TCP, Remote.ROBO_RIO, true, (ReceiveTagBase<String>) UsageReporting::build),
    DISABLE_FAULTS(0x04, Protocol.TCP, Remote.ROBO_RIO, true, (ReceiveTagBase<Integer>) (packet, size) -> new TagValueMap<Integer>()
    ...
    RAIL_FAULTS(0x05, Protocol.TCP, Remote.ROBO_RIO, true, (ReceiveTagBase<Integer>) (packet, size) -> new TagValueMap<Integer>()
    ...
    VERSION_INFO(0x0A, Protocol.TCP, Remote.ROBO_RIO, true, (ReceiveTagBase<String>) (packet, size) -> {
    ...
    ERROR_MESSAGE(0x0B, Protocol.TCP, Remote.ROBO_RIO, true, (ReceiveTagBase<String>) (packet, size) -> {
    ...
    STANDARD_OUT(0x0C, Protocol.TCP, Remote.ROBO_RIO, true, (ReceiveTagBase<String>) (packet, size) -> new TagValueMap<String>()
            .addTo("Timestamp", String.valueOf(NumberUtils.getFloat(ArrayUtils.sliceArr(packet, 0 ,4))))
            .addTo("Sequence Num", String.valueOf(NumberUtils.getUInt16(ArrayUtils.sliceArr(packet, 4, 6))))
            .addTo("Message", new String(ArrayUtils.sliceArr(packet, 6)))
    ),
    TCP_R2D_UNKNOWN(0x0D, Protocol.TCP, Remote.ROBO_RIO, true, (ReceiveTagBase<Byte>) TagValueMap::passPackets),

    WPILIB_VER(0x00, Protocol.TCP, Remote.FMS, true, VERSION_INFO.action),
    RIO_VER(0x01, Protocol.TCP, Remote.FMS, true, VERSION_INFO.action),
    DS_VER(0x02, Protocol.TCP, Remote.FMS, true, VERSION_INFO.action),
    PDP_VER(0x03, Protocol.TCP, Remote.FMS, true, VERSION_INFO.action),
    PCM_VER(0x04, Protocol.TCP, Remote.FMS, true, VERSION_INFO.action),
    CANJAG_VER(0x05, Protocol.TCP, Remote.FMS, true, VERSION_INFO.action),
    CANTALON_VER(0x06, Protocol.TCP, Remote.FMS, true, VERSION_INFO.action),
    THIRD_PARTY_DEVICE_VER(0x07, Protocol.TCP, Remote.FMS, true, VERSION_INFO.action),
    EVENT_CODE(0x14, Protocol.TCP, Remote.FMS, true, (ReceiveTagBase<String>) (packet, size) ->
            TagValueMap.singleton("Event Name", new String(packet))
    ),
    STATION_INFO(0x19, Protocol.TCP, Remote.FMS, true, (ReceiveTagBase<AllianceStation>) (packet, size) -> {
        String status = "";
    ...
    CHALLENGE_QUESTION(0x1A, Protocol.TCP, Remote.FMS, true, (ReceiveTagBase<Integer>) (packet, size) ->
            TagValueMap.singleton("Challenge Value", NumberUtils.getUInt16(
                    ArrayUtils.sliceArr(packet, packet.length - 2, packet.length)))
    ),
    GAME_DATA(0x1C, Protocol.TCP, Remote.FMS, true, (ReceiveTagBase<String>) (packet, size) ->
    ...
    private final boolean includeInLog;
    private final ReceiveTagBase<?> action;

    ReceiveTag(int flag, Protocol protocol, Remote remote, boolean includeInLog, ReceiveTagBase<?> action) {
        this.flag = flag;
