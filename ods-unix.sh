...
gh_latest_release=$(curl -s "https://api.github.com/repos/Boomaa23/open-ds/releases/latest")
...
if [[ -z "which java" ]]; then
    ...
    if [[ ! -d $dl_basename ]]; then
        ...
        curl -s -L "https://api.adoptopenjdk.net/v3/binary/latest/$jre_ver/ga/$unix_type/$req_arch/jre/hotspot/normal/adoptopenjdk?project=jdk" >> $dl_basename.tar.gz
